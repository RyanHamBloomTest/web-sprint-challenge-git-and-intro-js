...
function getArtistByIndex(artistArray, arrayIndex) {
  return `the artist at index ${arrayIndex} is ${artistArray[arrayIndex].name}`;
}
...
function listOfNames(artistArray) {
  return artistArray.map((artist) => artist.name);
}
...
function removeArtist(artistArray, arrayIndex) {
  return artistArray.filter((artist) => artist.id !== arrayIndex);
}
...
function addArtist(artistArray, name, years, genre, nationality, bio) {
  artistArray.push({name, years, genre, nationality, bio});
  return artistArray;
}
...
function lotsOfArt(artistArray) {
  return artistArray.filter((artist) => artist.paintings > 100).map((artist) => artist.name);
}
...
function artistInfo(artistArray, artistName){
  return artistArray.filter((artist) => artist.name === artistName)[0]['bio'];
}
...
function artistByCountry(artistArray, nationality){
  return artistArray.filter((artist) => artist.nationality === nationality).map((artist) => artist.name);
}
